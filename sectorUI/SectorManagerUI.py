...
class MainUI:
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.mapCanvas = Canvas(self.mapFrame, width=HEX_SIZE*1.7*GRID_COLUMNS, height=HEX_SIZE*math.sqrt(3)*(GRID_ROWS+1))
        self.mapCanvas.pack()
        ...
        for i in range(GRID_COLUMNS):
            for j in range(GRID_ROWS):
                ...
                new_hex = self.mapCanvas.create_polygon(hex_points[0][0], hex_points[0][1], hex_points[1][0], hex_points[1][1], hex_points[2][0], hex_points[2][1],
                                                        hex_points[3][0], hex_points[3][1], hex_points[4][0], hex_points[4][1], hex_points[5][0], hex_points[5][1], fill='white', outline='black')
    ...
    def show_grid_coords(self):
        for id in range(GRID_COLUMNS*GRID_ROWS):
            ...
            text_id = self.mapCanvas.create_text((hex_coord[0], hex_coord[1] - HEX_SIZE * math.sqrt(3)/3), text=coord_str, fill='gray')
    ...
    def place_star_on_map(self, name, coord):
        ...
        planet_text = self.mapCanvas.create_text((name_coord[0], name_coord[1] + HEX_SIZE * math.sqrt(3)/4), text=name, fill='gray')
        planet_oval = self.mapCanvas.create_oval((name_coord[0] - HEX_SIZE/5, name_coord[1] - HEX_SIZE/5, name_coord[0] + HEX_SIZE/5, name_coord[1] + HEX_SIZE/5), fill='gray')
